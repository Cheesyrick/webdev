...
<?php
    ...
    if(isset($_POST['Simpan']))
    {
        ...
        $ekstensifile = pathinfo($fotodestinasi, PATHINFO_EXTENSION);

    if(($ekstensifile == "jpg") or ($ekstensifile == "JPG"))
    {
        move_uploaded_file($file_tmp, 'images/'.$fotodestinasi);
        mysqli_query($connection, "insert into destinasik value ('$destinasiKODE', '$destinasiNAMA', '$destinasiKET', '$fotodestinasi', '$kategoriKODE')");
                ...
                <main>
                    <div class="container-fluid px-4">
                        ...
                        <ol class="breadcrumb mb-4">
                            <li class="breadcrumb-item active">Menginput data destinasi disini</li>
                        </ol>
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Website destinasik</title>
...
<body>
  ...
  <table class="table table-dark table-striped">
    ...
    <?php 
        ...
        {
            $search = $_POST["search"];
            $query = mysqli_query($connection,"select destinasik.* from destinasik where destinasiNAMA like '%".$search."%' limit $mulaitampilan, $jumlahtampilan");
            // --------------------- PAGING -- MEMBUAT GANTI HALAMAN ----------------
        }
        else 
        {
            $query = mysqli_query($connection,"select destinasik.* from destinasik limit $mulaitampilan, $jumlahtampilan");
        }
    ...
    <tr>
      ...
      <td><?php 
				if(is_file("images/".$row['fotodestinasi']))
				{
			?>
    ...
    <!--------------------- PAGING -- MEMBUAT GANTI HALAMAN ---------------->
    <?php 
        $query = mysqli_query($connection, "SELECT * FROM destinasik");
